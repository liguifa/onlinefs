...
module.exports = class folderServcie {
    constructor() {
    ...
    getFilesAndSubFoldersByFolderId(folderId, sortType, isAsc, searchKey) {
        var sortColumn = this._getSortColumnBySortType(sortType);
        var asc = isAsc === 1 ? "asc" : "desc";
        return sqlHelper.query(`select * from (select onlinefs_files.name as name,onlinefs_versions.size as size,onlinefs_filetypes.name as fileType, onlinefs_versions.version as version,onlinefs_users.displayName as modifyBy, onlinefs_files.modifyTime as modifyTime,1 as type
        ...
        where name like '%${searchKey}%' or '${searchKey}' = ''
